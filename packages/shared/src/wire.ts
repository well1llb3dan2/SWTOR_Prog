...
export const meterSnapshotSchema = z.object({
  ...
  bossFight: z.object({
    ...
    interrupts: z.array(z.object({
      timestamp: z.number().int().nonnegative(),
      abilityId: z.string().max(64),
      abilityName: z.string().max(128),
      sourceId: z.string().max(64).nullable(),
      sourceName: z.string().max(128).nullable(),
      targetNpcId: z.string().max(64).nullable(),
      targetName: z.string().max(128).nullable(),
